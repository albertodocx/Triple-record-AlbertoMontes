...
function [results] = BEH_PETHonsetv2(contvar, fscontvar, evsamp, fsev, combiname,  savepath, varargin)
...
addParameter(p,'bin', 0.1, @isnumeric); % PETH está a 10 Hz
...
[val, ~] = max(boutdur); Postwfr = val; 
PostWind = round(Postwfr*fsev);
Postcontvar = round(Postwfr*fscontvar);
% PostWind = Postwfr*fsev;
% 
% BLdata = zeros([size(Events, 1) PreWind]);
% EVdata = zeros([size(Events, 1) PostWind]);
% 
BLdata = zeros([size(Events, 1) Precontvar]);
EVdata = zeros([size(Events, 1) Postcontvar]);
...
for ii = 1:size(Events, 1)
    if ii == 1
        EVdata(ii, :) = contvar(tscontvar(ii, 1):tscontvar(ii, 1)+ Postcontvar -1);
        if (tscontvar(ii, 1) - Precontvar - 1) < 0 % que exceda por deltante
            ...
            % boutdurTS
            BLdata(ii, :) = [nan([1 abs(tscontvar(ii, 1) - Precontvar - 1)]) contvar(1:tscontvar(ii, 1)-1)];
        else % caso normal
            BLdata(ii, :) = contvar(tscontvar(ii, 1)-Precontvar:tscontvar(ii, 1)-1);
        end
    elseif ii > 1 && ii < size(Events, 1)
        if (tscontvar(ii, 1) - Precontvar - 1) < 0 % Que exceda por delante
            BLdata(ii, :) = [nan([1 abs(tscontvar(ii, 1) - Precontvar - 1)]) contvar(1:tscontvar(ii, 1)-1)];
            EVdata(ii, :) = contvar(tscontvar(ii, 1):tscontvar(ii, 1)+ Postcontvar-1);
        elseif (tscontvar(ii, 1) + Postcontvar) > length(contvar) % Que exceda por detrás
            nansize = (Postcontvar - length(contvar(tscontvar(ii, 1):end))) ;
            BLdata(ii, :) = contvar(tscontvar(ii, 1)-Precontvar:tscontvar(ii, 1)-1);
    ...
    elseif ii == size(Events, 1)
        if (tscontvar(ii, 1) + Postcontvar -1) > length(contvar)
            nansize = (Postcontvar - length(contvar(tscontvar(ii, 1):end))) ;
            BLdata(ii, :) = contvar(tscontvar(ii, 1)-Precontvar:tscontvar(ii, 1)-1);
            EVdata(ii, :) = [contvar(tscontvar(ii, 1):end), nan([1 nansize ])];
        else 
